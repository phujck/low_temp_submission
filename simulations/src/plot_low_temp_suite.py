...
# Enable LaTeX rendering and set font size
# User requested "much bigger" labels and legends. 
# Physical sizes are ~3.4" and ~7", so 10pt is "normal". 
# We boost to 12-16pt range to make them pop.
plt.rcParams.update({
    "text.usetex": True,
    "font.family": "serif",
    "font.size": 9,            # Base text size
    "axes.titlesize": 10,      # Panel titles (if any)
    "axes.labelsize": 10,      # Axis labels (Reduced from 14)
    "xtick.labelsize": 8,      # Tick labels (Reduced from 10)
    "ytick.labelsize": 8,      # Tick labels (Reduced from 10)
    "legend.fontsize": 8,      # Legend (Reduced from 10)
})
...
def plot_fig1_temp(data_dir, fig_dir):
    ...
    for idx, g in enumerate(gs):
        ...
        ax_a.plot(sub["beta"], sub["p4_path"], "--", color=color, linewidth=0.8, alpha=0.9)
        ax_a.plot(sub["beta"], sub["p4_scalar"], ":", color=color, linewidth=1.0, alpha=0.9)

    ax_a.set_ylabel(r"$p_4$")
    add_panel_label(ax_a, "(a)")
    
    # Legend - split content to avoid crowding
    method_handles = [
        Line2D([0], [0], color="k", lw=1.5, linestyle="-", label="analytic"),
        Line2D([0], [0], color="k", marker="o", lw=0.0, markersize=3.0, label="ED"),
        Line2D([0], [0], color="k", lw=0.8, linestyle="--", label="HS-path"),
    ]
    ax_a.legend(handles=method_handles, fontsize=8, loc="upper right", framealpha=0.8)

    # --- Panel B: lambda vs beta ---
    disc_scaled = np.median(arr1["lambda_disc"] / np.maximum(arr1["g2"], 1e-12))
    cont_scaled = np.median(arr1["lambda_cont"] / np.maximum(arr1["g2"], 1e-12))
    ax_b.axhline(disc_scaled, color="k", linestyle="--", linewidth=1.0, label=r"$\lambda_{\rm disc}/g^2$")
    ax_b.axhline(cont_scaled, color="gray", linestyle="-.", linewidth=1.0, label=r"$\lambda_{\rm cont}/g^2$")
    ...
    for idx, g in enumerate(gs):
        ...
        ax_b.plot(sub["beta"], sub["lambda_est_path"] / g2, "x--", color=color, linewidth=0.8, alpha=0.9)
        ax_b.plot(sub["beta"], sub["lambda_est_scalar"] / g2, "^:", color=color, linewidth=0.8, alpha=0.9)

    ax_b.set_xlabel(r"$\beta$")
    ax_b.set_ylabel(r"$\lambda_{\mathrm{est}}/g^2$")
    add_panel_label(ax_b, "(b)")
    
    # Combined legend for g values and refs in bottom panel
    g_handles = [Line2D([0], [0], color=colors[i], lw=1.5, label=fr"$g={gs[i]:.2f}$") for i in range(len(gs))]
    ax_b.legend(handles=g_handles, fontsize=8, loc="center right", framealpha=0.8)
...
def plot_fig2_strong(data_dir, fig_dir):
    ...
    ax_d.plot(arr2["g2"], arr2["fit_a_p"], "o-", linewidth=1.5, label=r"$a_P$")
    ax_d.plot(arr2["g2"], -arr2["lambda_disc"], "s--", linewidth=1.5, label=r"$-\lambda$")
    ax_d.plot(arr2["g2"], arr2["fit_a_hs"], "k:", linewidth=1.2, label=r"$a_{H_S}$")
...
def main():
    ...
    print("low_temp Gaussian figures generated (split 1, 2, 3)")
