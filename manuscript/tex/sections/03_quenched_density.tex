...
space, we define the \emph{quenched} imaginary-time propagator and its
normalisation by
\begin{equation}
\begin{split}
    \bar\rho(\beta)
    ...
    \\
    Z_\beta&:=\Tr\,\hat\tau \exp\!\left[-\int_{0}^{\beta} d\tau\,\bar H(\tau)\right],
\end{split}
...
induced by an external field is imposed only after \eqref{eq:quenched_canonical_def}
has been defined.

\subsection{Configuration-space path integral}
...
Discretise $[0,\beta]$ into $N$ slices of width $\Delta=\beta/N$ and define
...
\paragraph{Short-time factorisation.}
...
\end{align}
\begin{widetext}
Substituting \eqref{eq:kinetic_short_time} and \eqref{eq:short_time_kernel_split} into
\eqref{eq:quenched_kernel_product} yields
\begin{equation}
...
In the limit $N\to\infty$ (i.e.\ $\Delta\to 0$), \eqref{eq:quenched_discrete_PI} becomes the
