...
\end{align}
Eqs.~\eqref{eq:alpha_def}--\eqref{eq:hMF_beta_def} constitute an exact closed-algebra construction of
$H_{\mathrm{MF}}(\beta)$ for a qubit at arbitrary coupling strength: all bath dependence enters through the scalar moments
$M_{nm}(\beta)$ (equivalently through Eq.~\eqref{eq:Cbeta_def} or $J(\omega)$), while all system dependence enters through the finite adjoint-chain
...
\end{equation}
where $\alpha(\beta)$ is given explicitly by Eqs.~\eqref{eq:alpha_explicit} or Eq.~\eqref{eq:alpha_compact},
$\boldsymbol\delta(\beta)$ by Eqs.~\eqref{eq:delta_explicit} or Eq.~\eqref{eq:delta_compact},
and $\mathbf z(\beta)$ is obtained from the closed composition relations Eqs.~\eqref{eq:z_norm}--\eqref{eq:z_dir}
