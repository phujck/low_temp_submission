...
Therefore the Gaussian average $\mathbb E_\xi[e^{f X}] = \exp[\frac{1}{2}C(\beta)f^2]$ can be performed exactly, and hence, using Eq.~\eqref{eq:bar_rho_as_quenched_average},
\begin{equation}
...
$\Tr\,e^{-\beta H_{\mathrm{MF}}}=1$. The only nontrivial operator content is then the correction $-\lambda f^2$, which
...
$-\lambda f^2$ cancels it exactly. Then $H_{\mathrm{MF}}$ reduces to the bare system Hamiltonian $H_Q^{\mathrm{bare}}$, and the normalised mean-force Gibbs state coincides with the bare-system
