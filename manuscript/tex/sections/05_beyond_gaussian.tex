...
general: The noncommutativity of $H_Q$ and $f$, which forces
nontrivial time ordering, and non-Gaussian bath statistics, which
generate higher-order connected correlators beyond the two-point kernel
$K(\tau-\tau')$. The previous section addressed the first obstruction. Here
we address the second, showing how it enters the formalism through
...
$B:=\sum_k c_k x_k$ from Eq.~\eqref{eq:HI_model} need not produce Gaussian
...
series Eq.~\eqref{eq:ordered_cumulant} therefore terminates at $n=2$:
\begin{equation}
    \left\langle
...
Eq.~\eqref{eq:kernel_spectral} and the variance $C(\beta)$ of
Eq.~\eqref{eq:Cbeta_def}. The entire influence-functional construction of
Sec.~\ref{sec:influence_to_hmf}---including the closed-form
mean-force Hamiltonian Eq.~\eqref{eq:HMF_commuting_final}---is therefore
...
The cumulant expansion Eq.~\eqref{eq:ordered_cumulant} therefore provides
...
Hamiltonian is given by Eq.~\eqref{eq:HMF_commuting_final}. Otherwise,
$\Phi^{(4)}$ and, if needed, higher terms enter as perturbative
corrections to the effective action Eq.~\eqref{eq:Seff_cumulant}. In the
commuting sector these produce the additional operator corrections
$f^4, f^6,\ldots$ appearing in
Eq.~\eqref{eq:HMF_non_gaussian}, each with a coefficient that is
...
action Eq.~\eqref{eq:Seff_cumulant} is multilocal in imaginary time and can
...
Eq.~\eqref{eq:bar_rho_as_quenched_average}. The inner average is the
...
\end{align}
reproducing Eq.~\eqref{eq:HMF_non_gaussian} exactly. Both levels of the
...
analytically, but the nested representation Eq.~\eqref{eq:nested_quenched}
