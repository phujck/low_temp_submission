...
We work in the imaginary-time path integral formalism. The Euclidean definition of the reduced equilibrium density matrix $\bar{\rho}_S(\beta)$ is given by the matrix elements
\begin{equation}
    \langle q_f|\bar{\rho}_S(\beta)|q_i\rangle = \frac{1}{Z_{\mathrm{tot}}} \int \mathcal{D}q \mathcal{D}\mathbf{x} \, e^{-S_E[q,\mathbf{x}]}.
\end{equation}
This representation is an imaginary-time path integral~\cite{feynmanQuantumMechanicsPath1965, kleinertPathIntegralsQuantum2009}. While this representation makes exact treatment of the bath possible, it comes at a conceptual cost: the path
