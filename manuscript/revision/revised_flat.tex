...
The bath is modelled as a collection of harmonic oscillators,
\begin{equation}
...
Euclidean path integral~\cite{feynmanQuantumMechanicsPath1965,kleinertPathIntegralsQuantum2009,groscheHandbookFeynmanPath1998}, such that the reduced density (normalised by $Z_{\bar H}$) is given by
\begin{equation}
...
The precise level spacings are selected randomly and kept fixed across all $(\beta,g)$ simulations. For the bath, we use $J_g(\omega)=2 g^2\omega e^{-\omega/\omega_c}$, incorporating system-bath coupling strength directly into the parameter $g$. We choose $\omega_c=4$ for all simulations. This Ohmic spectrum is used only to provide a concrete numerical test case: the analytic commuting result depends on the bath through $\lambda$ alone, so any spectral density for which the reorganisation integral is well defined gives the same operator form. As a point of comparison, we further implement an explicit bath of $K$ harmonic modes $a_k$, coupling to the system through $\sum_k a_k +a_k^\dagger$. The bath spectrum is then mimicked by discretising the interval $\omega\in[0, \omega_{\max}]$ into mode bins with centres $\omega_k$ and widths $\Delta\omega_k$, setting $c_k^2=\frac{2}{\pi}J_g(\omega_k)\,\omega_k\,\Delta\omega_k$. This ensures that the discretised reorganisation scale $\lambda_{\mathrm{disc}}=\sum_k\frac{c_k^2}{2\omega_k^2}$ tracks the continuum expression $\lambda_{\mathrm{cont}}=\frac{1}{\pi}\int_0^\infty\frac{J_g(\omega)}{\omega}\,d\omega$~\cite{weissQuantumDissipativeSystems2012}. This provides an exact reference for the reduced state $\rho_Q^{\mathrm{ED}}=\Tr_B[e^{-\beta H_{\mathrm{tot}}}]/\Tr[e^{-\beta H_{\mathrm{tot}}}]$. 

Finally, we implement the stochastic formulation of the quenched density given by Eq.~\eqref{eq:bar_rho_as_quenched_average}. The Gaussian noise $\xi(\tau)$ is generated by convolution of white noise with the covariance matrix of the discretised kernel $K(\tau)$, a standard technique for sampling coloured noise (see e.g. \cite{mccaulDrivingSpinbosonModels2018a}). Each stochastic estimate uses $50{,}000$ path samples on a $32$-point imaginary-time grid. Accuracy is assessed both by the resulting standard errors and the convergence of the noise of $X=\int_0^\beta d\tau\,\xi(\tau)$ to its  predicted variance $2\beta\lambda_{\mathrm{disc}}$. Fig.~\ref{fig:variance_scaling} confirms this scaling, plotting the sample variance extracted from the stochastic simulations against $\beta$ (panel a) and $g^2$ (panel b).
