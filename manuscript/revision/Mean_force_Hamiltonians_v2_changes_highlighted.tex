...
%DIF DEL manuscript\revision\baseline_flat.tex   Mon Apr 27 20:09:59 2026
%DIF ADD manuscript\revision\revised_flat.tex    Mon Apr 27 17:45:03 2026
...
\title{Mean-Force Hamiltonians from Influence Functionals\texorpdfstring{\\\large changes\textunderscore highlighted}{ - changes highlighted}}
...
The bath is model\DIFaddbegin \DIFadd{l}\DIFaddend ed as a collection of harmonic oscillators,
\begin{equation}
...
(unnormalise\DIFaddbegin \DIFadd{d}\DIFaddend  ) equilibrium operator is
\begin{equation}
...
Euclidean path integral~\cite{feynmanQuantumMechanicsPath1965,kleinertPathIntegralsQuantum2009,groscheHandbookFeynmanPath1998}\DIFaddbegin \DIFadd{, such that the reduced density (normalised by $Z_{\bar H}$) is given by
...
While the representation \DIFaddbegin \DIFadd{in Eq.~}\DIFaddend \eqref{eq:reduced_effective_PI} is exact, it is nonlocal (bilocal) in imaginary time. This nonlocality can be resolved by a \DIFdelbegin \DIFdel{Hubbard - Stratonovich }\DIFdelend \DIFaddbegin \DIFadd{Hubbard-Stratonovich }\DIFaddend (HS) transformation \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{hubbardCalculationPartitionFunctions1959a, stratonovich1957QDistro}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{hubbardCalculationPartitionFunctions1959a, stratonovich1957QDistro, XuAnkerholdEPJST2025}}\hskip0pt%DIFAUXCMD
}\DIFaddend , which decouples the bilocal interaction by introducing an auxiliary Gaussian field $\xi(\tau)$. This step provides the bridge to an operator-level representation of $\bar{\rho}_Q$ as a Gaussian average over the local quenched canonical densities discussed in Sec.~\ref{sec:quenched}:
\begin{equation}
    \DIFdelbegin %DIFDELCMD < \begin{split}
%DIFDELCMD <     \bar%%%
\DIFdel{\rho}\DIFdelend \DIFaddbegin \bar{\rho}\DIFaddend _Q(\beta) \DIFdelbegin %DIFDELCMD < &%%%
\DIFdelend = \DIFdelbegin \DIFdel{\mathbb E}\DIFdelend \DIFaddbegin \DIFadd{\mathbb{E}}\DIFaddend _\xi\bigg[\DIFdelbegin %DIFDELCMD < \\
%DIFDELCMD <     &%%%
\DIFdel{\quad }\DIFdelend \hat{\mathcal{T}}\exp\!\left(-\int_0^\beta d\tau\,\bar{H}(\tau;\xi)\right) \bigg],
    \DIFdelbegin %DIFDELCMD < \end{split}
%DIFDELCMD <     %%%
\DIFdelend \label{eq:bar_rho_as_quenched_average}
\end{equation}
\DIFaddbegin \DIFadd{where $\bar{H}(\tau;\xi) = H_Q - \xi(\tau)f$ is the time-dependent quenched Hamiltonian.
}\DIFaddend The statistical properties of the auxiliary field $\xi(\tau)$ are set by the influence kernel \cite{mccaulPartitionfreeApproachOpen2017c,mccaulDrivingSpinbosonModels2018a,mccaulHowWinFriends2021b}
\begin{equation}
    \mathbb{E}_\xi[\xi(\tau)] = 0, \quad \mathbb{E}_\xi[\xi(\tau)\xi(\tau')] = K(\tau-\tau').
    \label{eq:noise_statistics}
\end{equation}
This \DIFaddbegin \DIFadd{Hubbard--Stratonovich unravelling is the same as that underlying partition-free stochastic Liouville formulations of Gaussian environments and equilibrated initial conditions~\mbox{%DIFAUXCMD
\cite{mccaulPartitionfreeApproachOpen2017c}}\hskip0pt%DIFAUXCMD
. Recent work also develops a time-local minimally extended-state-space embedding (QD-MESS) via influence-functional decomposition and Hubbard–Stratonovich unravelling, recovering Caldeira–Leggett and Smoluchowski-type limits as special cases~\mbox{%DIFAUXCMD
\cite{XuAnkerholdEPJST2025}}\hskip0pt%DIFAUXCMD
.
}

\DIFadd{Having performed this transformation, the }\DIFaddend operator representation maps directly to the quenched density framework. The challenge now lies in performing the average over $\xi$. To do so, we must isolate the stochastic term. Moving to the interaction picture with respect to $H_Q$, the propagator factorises as
\begin{equation}
...
Crucially, $C(\beta)$ is \emph{linear} in $\beta$, so the ratio $C(\beta)/(2\beta)=\lambda$ is temperature independent. \DIFaddbegin \DIFadd{The closed commuting-sector result depends on the bath only through this spectral integral; the Ohmic form used below is therefore a numerical benchmark choice rather than a restriction of the derivation.
}\DIFaddend 
...
\end{equation}
\DIFdelbegin \DIFdel{$\Tr\,e^{-\beta H_{\mathrm{MF}}}=1$. }\DIFdelend The only nontrivial operator content is \DIFdelbegin \DIFdel{then }\DIFdelend the correction $-\lambda f^2$, which is temperature independent.
...
\cite{campisiTalknerHanggi2009Solvable,talknerColloquiumStatisticalMechanics2020,cerisolaQuantumClassicalCorrespondence2024}. \DIFaddbegin \DIFadd{This equivalence is most easily understood }\DIFaddend through the link to Koopman von-Neumann dynamics. In real time, classical evolution is generated by the Liouville/Koopman operator $\mathcal L_H=\{\,\cdot\,,H\}$, \DIFaddbegin \DIFadd{where $\{\,\cdot\,,H\}$ denotes the classical Poisson bracket, }\DIFaddend with propagator $e^{t\mathcal L_H}$. \DIFdelbegin \DIFdel{For equilibrium }\DIFdelend \DIFaddbegin \DIFadd{To obtain Gibbs states from imaginary time }\DIFaddend however, the relevant generator \DIFdelbegin \DIFdel{is the Hamiltonianin the Gibbs state}\DIFdelend \DIFaddbegin \DIFadd{becomes the Hamiltonian}\DIFaddend . This is true in both the quantum and classical scenarios, but in the latter case all observable operators \DIFaddbegin \DIFadd{(and hence the Hamiltonian) }\DIFaddend commute identically. \DIFdelbegin \DIFdel{Hence the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend classical equilibrium construction \DIFdelbegin \DIFdel{always lies in }\DIFdelend \DIFaddbegin \DIFadd{will therefore always lie within }\DIFaddend the commuting sector \DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{of the fully quantum problem. For a fuller discussion of the dynamical distinctions between quantum and classical systems in both real and imaginary time, see ref.~\mbox{%DIFAUXCMD
\cite{McCaulZhdanovBondar2023WaveOperator}}\hskip0pt%DIFAUXCMD
.
}\DIFaddend 

A\DIFdelbegin \DIFdel{nother}\DIFdelend  slight (but interesting) \DIFdelbegin \DIFdel{consequence }\DIFdelend \DIFaddbegin \DIFadd{corollary }\DIFaddend of this result concerns integrable systems where the coupling operator $f$ is a conserved quantity ($[H_Q, f]=0$). In this case, the Hamiltonian of mean force $H_{\mathrm{MF}}$ shares the same eigenbasis as the bare system Hamiltonian. The strong coupling to the bath therefore does not mix the system energy levels, but rather induces a level-dependent energy shift $-\lambda f_n^2$, where $f_n$ are the eigenvalues of the coupling operator. The integrability of the open system is thus preserved in the sense that the equilibrium state remains diagonal in the energy basis of the isolated system, but depending on the specific structure of $f$, the bath can induce a non-trivial operator-level effect on the reduced equilibrium state. 

\DIFdelbegin \DIFdel{Interestingly }\DIFdelend \DIFaddbegin \DIFadd{A final important consequence concerns the relationship between the mean force correction, and the system counterterm that is often added to enforce translational invariance of the bath in the Caldeira-Leggett model. Explicitly, we see that }\DIFaddend $\lambda$ coincides with the static potential renormalisation $\frac{1}{2}\sum_k c_k^2/(m_k\omega_k^2)$\DIFdelbegin \DIFdel{of the Caldeira-Leggett model}\DIFdelend . If the standard counterterm $+\lambda f^2$ has been \DIFdelbegin \DIFdel{absorbed into }\DIFdelend \DIFaddbegin \DIFadd{added to }\DIFaddend $H_Q$ (as stated in Sec.~\ref{sec:model}), the bath-induced shift $-\lambda f^2$ cancels it exactly\DIFdelbegin \DIFdel{. In this case, $H_{\mathrm{MF}}$ reduces to the bare system Hamiltonian $H_Q^{\mathrm{bare}}$, and }\DIFdelend \DIFaddbegin \DIFadd{:
}\begin{equation}
    \DIFadd{H_Q = H_Q^{\mathrm{bare}}+\lambda f^2
    \quad\Longrightarrow\quad
    H_{\mathrm{MF}}(\beta)=H_Q^{\mathrm{bare}}+\mathrm{const}.
    \label{eq:counterterm_cancellation}
}\end{equation}
\DIFadd{Thus, in the commuting sector with the standard counterterm convention, }\DIFaddend the normalised mean-force Gibbs state coincides with the bare-system Gibbs state\DIFdelbegin \DIFdel{. In other words, in the commuting sector with translational invariance enforced, the }\DIFdelend \DIFaddbegin \DIFadd{: the }\DIFaddend bath has no \DIFaddbegin \DIFadd{residual }\DIFaddend operator-level effect on the reduced equilibrium state. Conversely, if no counterterm is included, $-\lambda f^2$ represents a genuine bath-induced static renormalisation of the system potential. Amusingly, this result closely aligns to the master equation folk wisdom where superior results can be obtained when one neglects \emph{both} counterterms and Lamb-shifts~\cite{correaPotentialRenormalisationLamb2025}. 

\DIFaddbegin \DIFadd{There is perhaps a deeper lesson hiding within this observation.  Consider that in the commuting sector, the system operator $f$ does not drive the bath dynamically, but merely labels a family of displaced harmonic equilibria. Tracing out the bath then asks a simple thermodynamic question: what is the free-energy change of a harmonic environment whose equilibrium position has been shifted by an amount proportional to $f$? Because the harmonic trace is invariant under this translation, the only remnant is the displacement free energy, $-\lambda f^2$. The counterterm is precisely the same object inserted with the opposite sign before the bath is traced out. Another equivalent perspective regards the counterterm as subtracting the static susceptibility (i.e. a Matsubara zero-frequency mode) contribution of the influence kernel. In the commuting case only this zero mode contributes to the HMF, as within the interaction picture $f$ is definitionally $\tau$-independent. From these perspectives, the cancellation between counterterm and commuting HMF correction may be regarded as definitionally necessary, rather than merely coincidental.
}

\DIFaddend Having established the quenched density representation exactly captures the reorganisation-energy result~\cite{campisiTalknerHanggi2009Solvable}, we now apply it to stochastic numerical evaluation of the equilibrium state.
...
\end{equation}
\DIFaddbegin \DIFadd{Although the benchmark uses the projector $P=|4\rangle\langle 4|$, this is not a special assumption in the derivation. For any commuting Hermitian coupling $f=\sum_n f_n |n\rangle\langle n|$, Eq.~\eqref{eq:HMF_commuting_final} gives level shifts $-\lambda f_n^2$. As such, the choice to project to a single level is purely one of convenience and interpretational clarity, as in the commuting system-interaction regime all bath-induced operator content is localised to a single sector, whose effect can be resolved directly in the populations and fitted HMF coefficients.
}

\begin{figure}[htbp]
\includegraphics[width=\columnwidth]{../../simulations/results/figures/lt_cg_fig4_variance.pdf}
\caption{\label{fig:variance_scaling} \DIFaddFL{Variance scaling. The sample variance $\sigma^2_\xi$ of the stochastic field grows linearly with inverse temperature $\beta$ (a) and linearly with the reorganisation energy $\lambda \propto g^2$ (b). Symbols are simulation results from $50{,}000$ path samples on a $32$-point imaginary-time grid; dotted lines are the theoretical prediction $2\beta\lambda_{\mathrm{disc}}$.}}
\end{figure}

\DIFaddend The precise level spacings are selected randomly and kept fixed across all $(\beta,g)$ simulations. For the bath, we \DIFdelbegin \DIFdel{set }\DIFdelend use $J_g(\omega)=2 g^2\omega e^{-\omega/\omega_c}$, incorporating system-bath coupling strength directly into the parameter $g$. We choose \DIFdelbegin \DIFdel{$\omega_c=1$ }\DIFdelend \DIFaddbegin \DIFadd{$\omega_c=4$ }\DIFaddend for all simulations. \DIFaddbegin \DIFadd{This Ohmic spectrum is used only to provide a concrete numerical test case: the analytic commuting result depends on the bath through $\lambda$ alone, so any spectral density for which the reorganisation integral is well defined gives the same operator form. }\DIFaddend As a point of comparison, we further implement an explicit bath of $K$ harmonic modes $a_k$, coupling to the system through $\sum_k a_k +a_k^\dagger$. The bath spectrum is then mimicked by discretising the interval \DIFdelbegin \DIFdel{$\omega\in[0, \omega_c]$ }\DIFdelend \DIFaddbegin \DIFadd{$\omega\in[0, \omega_{\max}]$ }\DIFaddend into mode bins with centres $\omega_k$ and widths $\Delta\omega_k$, setting $c_k^2=\frac{2}{\pi}J_g(\omega_k)\,\omega_k\,\Delta\omega_k$. This ensures that the discretised reorganisation scale $\lambda_{\mathrm{disc}}=\sum_k\frac{c_k^2}{2\omega_k^2}$ tracks the continuum expression $\lambda_{\mathrm{cont}}=\frac{1}{\pi}\int_0^\infty\frac{J_g(\omega)}{\omega}\,d\omega$~\cite{weissQuantumDissipativeSystems2012}. This provides an exact reference for the reduced state $\rho_Q^{\mathrm{ED}}=\Tr_B[e^{-\beta H_{\mathrm{tot}}}]/\Tr[e^{-\beta H_{\mathrm{tot}}}]$. 

Finally, we implement the stochastic formulation of the quenched density \DIFdelbegin \DIFdel{(}\DIFdelend \DIFaddbegin \DIFadd{given by }\DIFaddend Eq.~\DIFdelbegin \DIFdel{\ref{eq:bar_rho_as_quenched_average})}\DIFdelend \DIFaddbegin \DIFadd{\eqref{eq:bar_rho_as_quenched_average}}\DIFaddend . The Gaussian noise $\xi(\tau)$ is generated by convolution of white noise with the covariance matrix of the discretised kernel $K(\tau)$, a standard technique for sampling coloured noise (see e.g. \cite{mccaulDrivingSpinbosonModels2018a}). \DIFdelbegin \DIFdel{In the present model, the variance of the integrated field }\DIFdelend \DIFaddbegin \DIFadd{Each stochastic estimate uses $50{,}000$ path samples on a $32$-point imaginary-time grid. Accuracy is assessed both by the resulting standard errors and the convergence of the noise of }\DIFaddend $X=\int_0^\beta d\tau\,\xi(\tau)$ \DIFdelbegin \DIFdel{is predicted to be }\DIFdelend \DIFaddbegin \DIFadd{to its  predicted variance }\DIFaddend $2\beta\lambda_{\mathrm{disc}}$. Fig.~\ref{fig:variance_scaling} confirms this scaling, plotting the sample variance extracted from the stochastic simulations against $\beta$ (panel a) and $g^2$ (panel b).

\DIFdelbegin %DIFDELCMD < \begin{figure}[htbp]
%DIFDELCMD < \includegraphics[width=\columnwidth]{lt_cg_fig4_variance.pdf}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
%DIFDELCMD < \label{fig:variance_scaling} %%%
\DIFdelFL{Variance scaling. The sample variance $\sigma^2_\xi$ of the stochastic field grows linearly with inverse temperature $\beta$ (a) and linearly with the reorganisation energy $\lambda \propto g^2$ (b). Symbols are simulation results; dotted lines are the theoretical prediction $2\beta\lambda_{\mathrm{disc}}$.}}
%DIFAUXCMD
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend With this setup the core predictions of the HMF framework can be verified. 
Fig.~\ref{fig:temp_dep} confirms the thermodynamic consistency of the \DIFdelbegin \DIFdel{Quenched Density }\DIFdelend \DIFaddbegin \DIFadd{quenched density }\DIFaddend framework, showing that the effective Hamiltonian correctly captures the temperature dependence of the open system, with the extracted reorganisation energy matching the theoretical value. 

\begin{figure}[htbp]
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\columnwidth]{lt_cg_fig1_temp.pdf}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\columnwidth]{../../simulations/results/figures/lt_cg_fig1_temp.pdf}
\DIFaddendFL \caption{\label{fig:temp_dep} Temperature dependence and scaling. (a) Population of the coupled state ($|4\rangle$) vs $\beta$ for various couplings $g$. Circles (ED) match solid lines (analytic). (b) The extracted reorganisation energy $\lambda_{\mathrm{est}}$ collapsed by $g^2$ vs $\beta$. All curves hover near the theoretical thermodynamic limit (dashed lines), verifying that the bath's effect reduces to a single reorganisation energy $\lambda$.}
\end{figure}

 
 \DIFaddbegin \begin{figure}[htbp]
 \includegraphics[width=\columnwidth]{../../simulations/results/figures/lt_cg_fig2_strong.pdf}
 \caption{\label{fig:strong_coupling} \DIFaddFL{Strong-coupling benchmark. (a) Population $p_4$ vs $g^2$ at fixed $\beta$, showing excellent agreement over the full range. (b) Coefficients in the system basis vs $g^2$. The projector component $P_4=|4\rangle\langle 4|$ scales as $-\lambda$, while off-diagonal coherences are exponentially suppressed. This strong-coupling projector dominance is consistent with ultrastrong-coupling analyses of the mean-force Gibbs state~\mbox{%DIFAUXCMD
\cite{cresserWeakUltrastrongCoupling2021a}}\hskip0pt%DIFAUXCMD
. The bare system Hamiltonian $H_S$ remains unrenormalised (dotted), confirming $H_{\mathrm{MF}} = H_S - \lambda f^2$.}}
\end{figure}

\DIFaddend Fig.~\ref{fig:strong_coupling} demonstrates robustness in the strong-coupling regime, where the interaction strength rivals the system energy scales. To verify the operator structure of the HMF, we fit the numerically extracted $H_{\mathrm{MF}}$ to the ansatz $c_0 \mathbb{I} + a_{H_S} H_S + a_P P$. As shown in panel (b), the coefficient $a_P$ tracks the theoretical prediction $-\lambda$ perfectly, while $a_{H_S} \approx 1$ confirms that the bare system Hamiltonian is not \DIFdelbegin \DIFdel{renormalized. }%DIFDELCMD < 

%DIFDELCMD < \begin{figure}[htbp]
%DIFDELCMD < \includegraphics[width=\columnwidth]{lt_cg_fig2_strong.pdf}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
%DIFDELCMD < \label{fig:strong_coupling} %%%
\DIFdelFL{Strong-coupling benchmark. (a) Population $p_4$ vs $g^2$ at fixed $\beta$, showing excellent agreement over the full range. (b) Coefficients of the HMF in the system basis vs $g^2$. The projector component $P=|4\rangle\langle 4|$ scales as $-\lambda$ (orange/blue), while $H_S$ remains unrenormalised (dotted), confirming $H_{\mathrm{MF}} = H_S - \lambda f^2$.}}
%DIFAUXCMD
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{renormalised. }\DIFaddend Finally, Fig.~\ref{fig:microscopic} provides the microscopic resolution: the HMF modifies only the coupled subspace, leaving the orthogonal manifold untouched, as predicted by the operator structure of Eq.~\ref{eq:HMF_commuting_final}.

\begin{figure}[htbp]
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\columnwidth]{lt_cg_fig3_micro.pdf}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\columnwidth]{../../simulations/results/figures/lt_cg_fig3_micro.pdf}
\DIFaddendFL \caption{\label{fig:microscopic} Microscopic validation. (a) Detailed population snapshot at fixed $\beta, g$: \DIFdelbeginFL \DIFdelFL{"}\DIFdelendFL Analytic HMF \DIFdelbeginFL \DIFdelFL{" }\DIFdelendFL (blue) corrects the \DIFdelbeginFL \DIFdelFL{"Bare }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{bare }\DIFaddendFL Gibbs \DIFdelbeginFL \DIFdelFL{" }\DIFdelendFL (grey) to match ED (orange). (b) Effective energy shifts $\Delta_i$ for each level; only the coupled level ($|4\rangle$) shifts, by exactly $-\lambda$. Non-coupled levels remain unperturbed.}
\end{figure}
...
The exact result derived here is hardly novel, and in isolation can hardly justify the formal machinery used to derive it. In this regard, our treatment is incomplete. The power of the quenched density lies in its modularity, which naturally accommodates both more general environments \DIFdelbegin \DIFdel{, and system bath-couplings (where $[H_S, f] \neq 0$). In the latter case, the challenge shifts from the statistical generation of noise to the Lie-algebraic properties of the time-ordered effective propagator. The framework thus clearly delineates the statistical complexity of the bath from the dynamical complexity of the system response. In forthcoming work, we shall address these generalisations. Even in this simplified casehowever, we }\DIFdelend \DIFaddbegin \DIFadd{and system-bath couplings. While we defer the full treatment to future work, it is possible to sketch the route to generalisation. 
}

\DIFadd{For non-commuting couplings, the object to control is the interaction-picture operator $f(\tau)=e^{\tau H_Q}fe^{-\tau H_Q}$. A natural extension is to expand it in the adjoint chain $f_n=\mathrm{ad}_{H_Q}^n(f)$, so that the Gaussian bath enters through scalar kernel moments multiplying products $f_n f_m$. This turns the problem of finding the HMF into a question of closure on the Lie Algebra generated by products of adjoint operators $f_n$ and $H_Q$. When this algebra closes, there exists a finite representation of the HMF. In the sequel this result will be demonstrated explicitly, but we see its shadow even in the present commuting case, where the algebra closes trivially and the HMF reduces to a static potential renormalisation. In this sense, we can already }\DIFaddend see the shape of the answer to the question with which we began. What governs equilibrium? Algebra.
