...
\cite{campisiTalknerHanggi2009Solvable,talknerColloquiumStatisticalMechanics2020,cerisolaQuantumClassicalCorrespondence2024}.
For a classical phase-space coordinate $z$,
\begin{equation}
\begin{split}
    H_{\mathrm{MF}}^{\mathrm{cl}}(z;\beta)
    &=
    -\frac{1}{\beta} \\
    &\quad \times \log\!\left[
    \frac{\int dX\,e^{-\beta\left(H_Q(z)+H_X(X)+H_{\mathrm{int}}(z,X)\right)}}
    {\int dX\,e^{-\beta H_X(X)}}
    \right],
\end{split}
    \label{eq:hmf_classical_def_sec3}
\end{equation}
and for Gaussian linear coupling this yields the same quadratic correction $-\kappa_0 f(z)^2/2$ up to a scalar shift. This equilvance can be understand mostly easily through the link to Koopman von-Neumann dynamics. In real time, classical evolution is generated by the Liouville/Koopman operator $\mathcal L_H=\{\,\cdot\,,H\}$, with propagator $e^{t\mathcal L_H}$.
For equilibrium however, the relevant generator is the Hamiltonian in the Gibbs state. This is true in both the quantum and classical scenarios, but in the latter case all observable operators commute identically. Hence the classical equilibrium construction always lies in the commuting sector.
...
\caption{\label{fig:strong_coupling} Strong-coupling benchmark. (a) Population $p_4$ vs $g^2$ at fixed $\beta$, showing excellent agreement over the full range. (b) Coefficients of the HMF in the system basis vs $g^2$. The projector component $P=|4\rangle\langle 4|$ scales as $-\lambda$ (orange/blue), while $H_S$ remains unrenormalised (dotted), confirming $H_{\mathrm{MF}} = H_S - \lambda f^2$.}
\end{figure}
